...
(async function registerFunction() {
  ...
  try {
    ...
    const proposalStr = `(record { title="Register ${functionName} function."; url="https://modclub.ai/"; summary="This proposal registers ${functionName} function."; action=opt variant {AddGenericNervousSystemFunction = record {id=${functionId}:nat64; name="${functionName}"; description=null; function_type=opt variant {GenericNervousSystemFunction=record{validator_canister_id=opt principal"${canisterId}"; target_canister_id=opt principal"${canisterId}"; validator_method_name=opt"${
      validateFunction === "yes" ? "validate" : validateFunction
    }"; target_method_name=opt"${functionName}"}}}}})`;
