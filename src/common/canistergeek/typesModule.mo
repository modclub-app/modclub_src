...
module {
  ...
  public type DayUpdateCallsCountData = [var Nat64];
  // number of update calls in each time interval for a specific day. E.g. array with 288 elements for 5-minute intervals.
  public type DayCanisterHeapMemorySizeData = [var Nat64];
  // canister heap memory size in each time interval for a specific day. E.g. array with 288 elements for 5-minute intervals.
  public type DayCanisterMemorySizeData = [var Nat64];
  // canister memory size in each time interval for a specific day. E.g. array with 288 elements for 5-minute intervals.
  public type DayCanisterCyclesData = [var Nat64];
  // canister available cycles in each time interval for a specific day. E.g. array with 288 elements for 5-minute intervals.
  ...
  public type DayDataId = Nat32;
  // composite key of the day data: 8 bits - year, 4 bits - month, 8 bits - day
  private let dayDataIdKeyEq : (DayDataId, DayDataId) -> Bool = func(x, y) {
    x == y;
  };
  private let dayDataIdKeyHash : (DayDataId) -> Hash.Hash = func(x) {
    Hash.hash(Nat32.toNat(x));
  };
  ...
  public func toDayDataId(year : Nat, month : Nat, day : Nat) : ?DayDataId {
    do ? {
      ...
      if (year >= MINIMAL_VALID_YEAR) {
        let yearIndex : Nat = year - MINIMAL_VALID_YEAR;
        //offset 2000 to reduce number of bits for year
  ...
  private func createCanisterMonitoringStateFromMigratedUpgradeData(
    ...
    switch (upgradeData) {
      case (#v1 value) {
        ...
        let newDayDataMap : DayDataMap = HashMap.fromIter<DayDataId, DayData>(
          dayDataIter,
          count * 2,
          dayDataIdKeyEq,
          dayDataIdKeyHash
