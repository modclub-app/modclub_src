...
// Assets
static ASSET_DIR: Dir<'_> = include_dir!("$CARGO_MANIFEST_DIR/../../dist/vc_rust");
pub fn init_assets() {
...
async fn prepare_credential(
    ...
    println!(
        "*** [DEBUG] alias_tuple.id_alias {:?}",
        alias_tuple.id_alias.to_text()
    );
    println!(
        "*** [DEBUG] alias_tuple.id_dapp {:?}",
        alias_tuple.id_dapp.to_text()
    );
    println!(
        "*** [DEBUG] credential_type {:?}",
        credential_type
    );
...
#[update]
#[candid_method]
async fn register_certification_platform(origin: String, canister_id: Principal) -> Result<bool, String> {
    println!(
        "*** [DEBUG] [register_certification_platform] [ORIGIN] {:?} \n [CANISTER_ID] {:?} \n",
        origin, canister_id
    );
...
#[update]
#[candid_method]
async fn check_certificate(origin: String, user_id: Principal) -> Result<bool, String> {
    println!(
        "*** [DEBUG] [check_certificate] [ORIGIN] {:?} \n [User_ID] {:?} \n",
        origin, user_id
    );
...
fn prepare_credential_payload(
    ...
    match credential_type {
        ProofOfHumanity(poph_vc) => {
            POH_VERIFIED.with_borrow(|poph_verified| {
