...
export const UnstakeHistoryTable = ({
  ...
  const pendingStakeValue = pendingStakeList
    ? Object.values(pendingStakeList)
    : [];
  return (
          ...
          {pendingStakeValue.length ? (
            pendingStakeValue.map((item) => (
              <tr key={item.created_at_time}>
                ...
                <td>
                  {convert_to_mod(BigInt(item.amount), BigInt(digit))} MOD{" "}
                </td>
                <td>
                  <Timer
                    countdown={getCountdown(
                      Number(item.created_at_time),
                      Number(item.dissolveDelay[0])
                    )}
                    detail={true}
                    toggle={undefined}
                  />
