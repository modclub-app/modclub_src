...
const toggleModal = () => {
  ...
  const getClaimableAmount = (nft) => {
    ...
    return nft.claimCount > 11 ? 0 : Math.floor(tier[Object.keys(nft.tier)[0]] / 12);
  };

  const getTotalLockedAmount = (nft) => {
    return Math.floor(tier[Object.keys(nft.tier)[0]] - (Number(nft.claimCount) * (tier[Object.keys(nft.tier)[0]] / 12)));
  };
  ...
  <span className="info-icon" onClick={toggleModal}>&#8505;</span>
</div>
      {isLoggedIn ? (
    ...
    <>
      ...
      <div className="info-modal">
        <span className="close-icon" onClick={toggleModal}>&times;</span>
        <h3>How to claim</h3>
        <p className="">You can claim your airdrop once per month until paid in full. </p>
        <p>Claims are soley based on holding the NFT, this means you can sell or trade the NFT, and the remaining monthly claims will be available to the new owner.</p>
        <h3>Account</h3>
...
{NFTs.length > 0 ? (
  <div className="nft-cards">
    {NFTs.map((nft, index) => (
      <div key={index} className="nft-card" style={getStatus(nft) === "timeLocked" ? {filter: "brightness(0.5)"} : {}}>
        <span className="nft-number">#{nft.tokenIndex}</span>
        <div>
        <img src={getTierImage(Object.keys(nft.tier)[0])} alt={Object.keys(nft.tier)[0]} className="nft-image" />
        </div>
        <div className="nft-card-info">
          <p className={getStatus(nft) === "timeLocked" ? "not-claimable" : "claimable"}>
            {getStatus(nft) === "timeLocked" ? `Claim available in ${BigInt(nft.dissolveDelay / BigInt(86400))} days` : "Claim Available"}
          </p>
          
          <div className="tokens">
            <img src={logo} alt="logo" className="card-icon" />
            <p className="text-small"> <strong>Claimable:</strong> {getClaimableAmount(nft)} MOD</p>    
          </div>
          <div className="tokens">
            <img src={coins} alt="logo" className="card-icon" />
            <p className="text-small"><strong>Total Claimed:</strong> {getTotalClaimed(nft)} MOD</p>
          </div>

          <div className="tokens">
            <img src={lockIcon} alt="Lock Icon" className="card-icon" />
            <p className="text-small"><strong>Total Locked: </strong>{getTotalLockedAmount(nft)} MOD</p>
          </div>
        
            <div className="tokens">
              <img src={calendarIcon} alt="Calendar Icon" className="card-icon" />
              {nft.lastClaim.timeStamp > 0 ? 
              <p className="text-small"><strong>Last Claimed:</strong> {new Date(Number(nft.lastClaim.timeStamp) / 1000000).toLocaleDateString()}</p>
              :
              <p className="text-small"><strong>Last Claimed:</strong> N/A</p>
              }
            </div>
        </div>
        <button onClick={() => handleClaimForNFT(nft.tokenIndex)} disabled={(getStatus(nft) === "timeLocked") || buttonLoading === true ? true : false} className={(getStatus(nft) === "timeLocked") || buttonLoading ? "disabled" : "" }>
          {buttonLoading ? <div className="loader is-loading"></div> : "Claim Tokens"}
        </button>
      </div>
    ))}
  </div>
