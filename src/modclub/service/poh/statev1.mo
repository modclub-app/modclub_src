...
module State {
  ...
  public type PohStableState = {
    ...
    pohUserChallengeAttempts : [
      (Principal, [(Text, [PohTypes.PohChallengesAttempt])])
    ];
  ...
  public func getState(stableState : PohStableState) : PohState {
    ...
    for ((userId, challenges) in stableState.pohUserChallengeAttempts.vals()) {
      let attemptsByChallengeIdMap = HashMap.HashMap<Text, Buffer.Buffer<PohTypes.PohChallengesAttempt>>(
        5,
        Text.equal,
        Text.hash,
      );
      for ((challengeId, attempts) in challenges.vals()) {
        let attemptsBuffer = Buffer.Buffer<PohTypes.PohChallengesAttempt>(
          attempts.size(),
        );
  ...
  public func getStableState(state : PohState) : PohStableState {
    ...
    let pohUserChallengeAttempts = Buffer.Buffer<(Principal, [(Text, [PohTypes.PohChallengesAttempt])])>(
      state.pohUserChallengeAttempts.size(),
    );
    for ((userId, challenges) in state.pohUserChallengeAttempts.entries()) {
      let challengeMap = Buffer.Buffer<(Text, [PohTypes.PohChallengesAttempt])>(
        challenges.size(),
      );
    ...
    let st = {
      ...
      providerToModclubUser = Iter.toArray(
        state.providerToModclubUser.entries(),
      );
      ...
      provider2PohVerificationRequests = Iter.toArray(
        state.provider2PohVerificationRequests.entries(),
      );
      pohVerificationRequests = Iter.toArray(
        state.pohVerificationRequests.entries(),
      );
    };
    return st;
  };
