...
module AuthModule {
  ...
  public func setUpDefaultAdmins(
    ...
    if (
      not List.some<Principal>(
        admins,
        func(val : Principal) : Bool { Principal.equal(val, initializer) },
      ),
    ) {
    ...
    if (
      not List.some<Principal>(
        ...
        func(val : Principal) : Bool {
          Principal.equal(val, mainActorPrincipal);
        },
      ),
    ) {
  ...
  public func isAdmin(caller : Principal, admins : List.List<Principal>) : Bool {
    ...
    var exists = List.find<Principal>(
      admins,
  ...
  public func registerAdmin(
    ...
    admins : List.List<Principal>,
    id : Principal,
  ) : Result.Result<List.List<Principal>, Text> {
