...
module {
    ...
    public func download(state: StorageState.DataCanisterState, varName: Text, start: Int, end: Int): [[Text]] {
        switch(varName) {
            case("dataCanisters") {
    ...
    func serializeDataCanisters(dataCanisters : HashMap.HashMap<StorageTypes.DataCanisterId, Buckets.Bucket>) : [[Text]] {
        let buff = Buffer.Buffer<[Text]>(1);
        for((canId, canister) in dataCanisters.entries()) {
            buff.add(
                [Principal.toText(canId), Principal.toText(Principal.fromActor(canister))]
            );
        };
        return buff.toArray();
    };

    func serializeContentIdToCanisterId(contentIdToCanisterId : HashMap.HashMap<Text, StorageTypes.DataCanisterId>) : [[Text]] {
        let buff = Buffer.Buffer<[Text]>(1);
    ...
    func serializeModeratorsId(moderatorsId : HashMap.HashMap<Principal, Principal>) : [[Text]] {
        let buff = Buffer.Buffer<[Text]>(1);
