...
module {
    ...
    public func download(state: GlobalState.State, varName: Text, start: Int, end: Int): [[Text]] {
        switch(varName) {
            case("GLOBAL_ID_MAP") {
    ...
    func serializeProvidersWhitelist(providersWhitelist : HashMap.HashMap<Principal, Bool>) : [[Text]] {
        let buff = Buffer.Buffer<[Text]>(1);
    ...
    func serializeProviders(providers : HashMap.HashMap<Principal, Types.Provider>) : [[Text]] {
        let buff = Buffer.Buffer<[Text]>(1);
        for((pId, providerDetail) in providers.entries()) {
            buff.add(
                [Principal.toText(pId), DownloadUtil.joinArr(DownloadUtil.toString_Provider([providerDetail]))]
            );
        };
        return buff.toArray();
    };

    func serializeProviderAdmins(providerAdmin : HashMap.HashMap<Principal, HashMap.HashMap<Principal, ()>>) : [[Text]] {
        let buff = Buffer.Buffer<[Text]>(1);
    ...
    func serializeProfiles(profiles : HashMap.HashMap<Principal, Types.Profile>) : [[Text]] {
        ...
        for((pId, profile) in profiles.entries()) {
            buff.add(
                [Principal.toText(pId), DownloadUtil.joinArr(DownloadUtil.toString_Profile([profile]))]
            );
        };
        return buff.toArray();
    };

    func serializeAirdropWhitelist(airdropWhitelist : HashMap.HashMap<Principal, Principal>) : [[Text]] {
        let buff = Buffer.Buffer<[Text]>(1);
    ...
    func serializeRelPrincipal2Principal(mods2votes : RelObj.RelObj<Principal, Principal>) : [[Text]] {
        let buff = Buffer.Buffer<[Text]>(1);
        for(userId in mods2votes.getKeys().vals()) {
            for(vId in mods2votes.get0(userId).vals()) {
                buff.add(
    ...
    func serializeProvider2PohExpiry(GLOBAL_ID_MAP : HashMap.HashMap<Principal, Nat>) : [[Text]] {
        let buff = Buffer.Buffer<[Text]>(1);
        for((category, id) in GLOBAL_ID_MAP.entries()) {
            buff.add(
    ...
    func serializeProvider2PohChallengeIds(provider2PohChallengeIds : HashMap.HashMap<Principal, Buffer.Buffer<Text>>) : [[Text]] {
        let buff = Buffer.Buffer<[Text]>(1);
    ...
    func serializeAirdropUsers(airdropUsers : HashMap.HashMap<Principal, Types.AirdropUser>) : [[Text]] {
        let buff = Buffer.Buffer<[Text]>(1);
        for((uId, user) in airdropUsers.entries()) {
            buff.add(
                [Principal.toText(uId), DownloadUtil.joinArr(DownloadUtil.toString_AirdropUser([user]))]
            );
        };
        return buff.toArray();
    };
    ...
    func serializeTextContent(textContents : HashMap.HashMap<Text, Types.TextContent>) : [[Text]] {
        let buff = Buffer.Buffer<[Text]>(1);
        for((cId, textContent) in textContents.entries()) {
            buff.add(
                [cId, DownloadUtil.joinArr(DownloadUtil.toString_TextContent([textContent]))]
            );
        };
        return buff.toArray();
    };

    func serializeImageContent(imageContents : HashMap.HashMap<Text, Types.ImageContent>) : [[Text]] {
        let buff = Buffer.Buffer<[Text]>(1);
        for((cId, imageContent) in imageContents.entries()) {
            buff.add(
                [cId, DownloadUtil.joinArr(DownloadUtil.toString_ImageContent([imageContent]))]
            );
        };
        return buff.toArray();
    };

     func serializeProviderSubs(providerSubs : HashMap.HashMap<Principal, Types.SubscribeMessage>) : [[Text]] {
        let buff = Buffer.Buffer<[Text]>(1);
