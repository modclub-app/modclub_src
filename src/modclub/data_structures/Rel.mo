...
module {
  ...
  public func fromShare<X, Y>(
    rel : RelShared<X, Y>,
    hash_ : HashPair<X, Y>,
    equal_ : EqualPair<X, Y>,
  ) : Rel<X, Y> {
    let temp = {
      forw = rel.forw;
      back = Trie.empty();
  ...
  public func isMember<X, Y>(rel : Rel<X, Y>, x : X, y : Y) : Bool {
    switch (
      Trie.find<X, Trie.Trie<Y, ()>>(rel.forw, keyOf0(rel, x), rel.equal.0),
    ) {
      case null false;
      case (?t) {
  ...
  func iterAll<K>(t : Trie.Trie<K, ()>) : Iter.Iter<K> = object {
