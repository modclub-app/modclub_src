...
import {Ecdsa,PrivateKey,PublicKey} from 'starkbank-ecdsa';
...
async function getBufferFromBlob(argFile){
    const genArrayBuffer = await argFile.arrayBuffer();
...
export class Secp256k1KeyIdentity extends SignIdentity {
    ...
    static fromParsedJson(obj) {
        ...
        return new Secp256k1KeyIdentity(Secp256k1PublicKey.fromDer(toHexString(publicKeyDer)), PrivateKey.fromDer(toHexString(privateKeyDer)));
    }
    static fromJSON(json) {
        ...
        if (Array.isArray(parsed)) {
            ...
            const pk = publicKey
                ? Secp256k1PublicKey.fromRaw(new Blob(new Uint8Array(publicKey.data)))
                : Secp256k1PublicKey.fromDer(new Blob(new Uint8Array(_publicKey.data)));
            if (publicKey && secretKey && secretKey.data) {
                return new Secp256k1KeyIdentity(pk, new Blob(new Uint8Array(secretKey.data)));
            }
            else if (_publicKey && _privateKey && _privateKey.data) {
                return new Secp256k1KeyIdentity(pk, new Blob(new Uint8Array(_privateKey.data)));
            }
        }
     ...
     */
    toJSON() {
        const pubBuffer = getBufferFromBlob(this._publicKey.toDer());
        const prBuffer = getBufferFromBlob(this._privateKey.toDer());
        return [pubBuffer.toString('hex'), prBuffer.toString('hex')];
    }
    /**
    ...
    //         ? new Blob(challenge)
    //         : new Blob(new Uint8Array(challenge));
    ...
    //     return new Blob(new Uint8Array([...Array.from(new Uint8Array(ra)), ...Array.from(new Uint8Array(sa))]));
    // }
}
