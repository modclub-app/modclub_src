export const idlFactory = ({ IDL }) => {
  ...
  const PohStableState = IDL.Record({
    ...
    providersCallback: IDL.Vec(IDL.Tuple(IDL.Principal, SubscribePohMessage)),
  ...
  const ProfileInfoStable = IDL.Record({
    id: UserId,
    userName: IDL.Text,
    createdAt: Timestamp__1,
    role: Role,
  ...
  const ProviderAdminsInfo = IDL.Record({
    pid: IDL.Principal,
    admins: IDL.Vec(ProfileInfoStable),
  });
  ...
  const AccountsImportPayload = IDL.Record({
    adminsByProvider: IDL.Vec(ProviderAdminsInfo),
  ...
  const ModClub = IDL.Service({
    ...
    getImportedUsersStats: IDL.Func([], [Result_2], ["query"]),
    ...
    translateUserPoints: IDL.Func([], [Result_2], []),
