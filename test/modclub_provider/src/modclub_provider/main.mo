...
shared ({caller = initializer}) actor class ModclubProvider () = this {
    ...
    public shared({ caller }) func test() : async Text {
        ...
        // Update settings
        await MC.updateSettings(Principal.fromActor(this), {minVotes = 2; minStaked = 15});
    ...
    public func updateSettings(voteNum: Nat, stakeNum: Nat) : async () {
        await MC.updateSettings(initializer, {minVotes = voteNum; minStaked = stakeNum});
    };
